...
def get_mouse_group():
    mouse_grp = input('enter mouse group (e.g. wt_d1, drd_d2, etc ...): ')
    if mouse_grp != '':
        print(f'mouse group is {mouse_grp}')
        return mouse_grp
    else:
        mouse_grp = get_mouse_group().lower()
        return mouse_grp


def get_mouse_id():
    mouse_id = input('enter mouse ID (e.g. D4): ')
    if mouse_id != '':
        print(f'mouse ID is {mouse_id}')
        return mouse_id
    else:
        mouse_id = get_mouse_id().lower()
        return mouse_id

mouse_group = get_mouse_group()
...
# check if the mouse group already exists
group_file_path = os.path.join(file_path, mouse_group)
if os.path.exists(group_file_path):
    file_path = os.path.join(file_path, mouse_group)
else:
    os.system(f'mkdir {group_file_path}')
    file_path = group_file_path

dir_list = os.listdir(file_path)
timestamp = time.localtime()
timestamp = str(timestamp[0])[-2:1]+str(timestamp[1]).zfill(2)+str(timestamp[2]).zfill(2) # get year, month, day

new_base_folder_name = mouse_id + '_session'+timestamp
new_base_folder_name = f'{mouse_group}_{mouse_id}_session_{timestamp}'
        ...
        while i < num_images:
            ...
            try:
                    ...
                    if i == 0:
                        ...
                        try:
                            ...
                            if stop_event.is_set():
                                ...
                                timeout_count += 1
                                print(f'grab timeout #{timeout_count} while grabbing frame {i+1} , continuing...', end='\r')
                ...
                if primary:
                    ...
                    print('COLLECTING IMAGE ' + str(i + 1).zfill(len(str(num_images))) + ' of ' + str(num_images) + filler, end='\r')
                    sys.stdout.flush()
            ...
            with open(log_savepath, 'w') as f:
                with redirect_stdout(f):
                    for i, segment in enumerate(segments):
                        ...
                        print(f'_______Segment {i}________')
    ...
    try:
        ...
        if PySpin.IsWritable(nodemap.GetNode("GainAuto")):
            ...
            if gain_auto and gain_auto.GetEntryByName("Once"):
                ...
                print("AutoGain set to Once")
        else:
            print("not changing gain settings")
